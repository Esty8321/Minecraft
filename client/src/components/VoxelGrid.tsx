...
// 💡 נקרא תמיד מ-VITE_GAME_WS (ולא VITE_WS_URL)
const WS_URL = import.meta.env.VITE_GAME_WS as string | undefined;
...
interface VoxelGridProps {
  serverUrl?: string; // אם רוצים לעקוף באמצעות prop
}
...
const VoxelGrid: React.FC<VoxelGridProps> = ({ serverUrl }) => {
  ...
  const wsRef = useRef<WebSocket | null>(null);
  const reconnectTimeoutRef = useRef<number | null>(null); // בדפדפן עדיף number

  // ✅ קובעים URL ברור ל-WS. לא בונים מה-host של הדפדפן.
  const getWebSocketUrl = useCallback(() => {
    if (serverUrl) return serverUrl;
    if (WS_URL) return WS_URL;
    // Fallback בטוח – לשימוש בזמן פיתוח כשלא הוגדר .env
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data);
          if (data.type === "matrix") {
            setGameState({ w: data.w, h: data.h, data: data.data });
            const players = data.data.filter((cell: number) => (cell & 1) === 1);
            setPlayerCount(players.length);
          }
        } catch (err) {
      ...
      ws.onclose = () => {
        ...
        // ניסיון התחברות מחדש אחרי 3 שניות
  ...
  const renderGrid = () => {
    ...
    for (let r = 0; r < gameState.h; r++) {
      for (let c = 0; c < gameState.w; c++) {
        ...
        const cell = gameState.data[i];
        const isPlayer = (cell & 1) === 1;
        cells.push(
          <div
            key={`${r}-${c}`}
            className={`voxel-cell ${isPlayer ? "voxel-player" : "voxel-empty"}`}
            style={{ backgroundColor: isPlayer ? getColorFromBits(cell) : "transparent" }}
          />
        );
      }
    }
