...
// // Read from env (support both names)
// const WS_URL =
//   (import.meta.env.VITE_GAME_WS as string | undefined) ||
...
//   serverUrl?: string; // allow override via prop
// }
...
//   // CSS color from our 2-bit-per-channel encoding
...
//     const r = get2Bits(value, 2, 5); // R0=2, R1=5
//     const g = get2Bits(value, 3, 6); // G0=3, G1=6
//     const b = get2Bits(value, 4, 7); // B0=4, B1=7
...
//             // cells with BIT_IS_PLAYER (bit 0) set
...
// client/src/components/VoxelGrid.tsx
import React, { useEffect, useState, useCallback, useRef } from "react";
import { Wifi, WifiOff, Users, Gamepad2, Palette } from "lucide-react";
import { authStorage } from "../utils/auth";

// נקרא קודם כל מ-VITE_GAME_WS; אם אין, ניפול לכתובת מה-Edge באותו הוסט
...
const VoxelGrid: React.FC<VoxelGridProps> = ({ serverUrl }) => {
  ...
  // חילוץ צבע מ-2 ביטים לכל ערוץ
  const getColorFromBits = useCallback((value: number) => {
    const getBit = (v: number, bit: number) => (v >> bit) & 1;
    const get2Bits = (v: number, b0: number, b1: number) =>
      (getBit(v, b1) << 1) | getBit(v, b0);
    const r = get2Bits(value, 2, 5);
    const g = get2Bits(value, 3, 6);
    const b = get2Bits(value, 4, 7);
    const map = [0, 85, 170, 255];
    return `rgb(${map[r]}, ${map[g]}, ${map[b]})`;
  }, []);
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      ...
      console.log("[WS URL]", url, "token?", Boolean(token));
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === "matrix") {
            ...
            // BIT_IS_PLAYER = 0 => בדיקה עם (cell & 1)
            const players = data.data.filter((cell: number) => (cell & 1) === 1);
            setPlayerCount(players.length);
          }
        } catch (err) {
